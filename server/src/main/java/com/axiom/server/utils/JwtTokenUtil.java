...
public class JwtTokenUtil {
    ...
    private void warmUpJWT() {
        try {
            ...
            String warmupAccessToken = Jwts.builder().subject("warmup-user").issuer(ISSUER).audience()
                    .add("axiom-client").and().claim(CLAIM_USER_ID, 999L).claim(CLAIM_TWO_FACTOR, false)
            ...
            String warmupRefreshToken = Jwts.builder().subject("warmup-user").issuer(ISSUER).audience()
                    .add("axiom-client").and().claim(CLAIM_USER_ID, 999L).claim(CLAIM_TOKEN_TYPE, "refresh")
    ...
    public Claims parseAndValidateToken(String token) {
        try {
            Claims claims = Jwts.parser().verifyWith(secretKey).requireIssuer(ISSUER).requireAudience("axiom-client")
    ...
    private static final String ISSUER = "axiom-server";
